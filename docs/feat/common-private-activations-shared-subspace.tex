...
Each side is globally average-pooled over tokens:
\[
  \bm{q}^{\mathrm{L}}_{p}[b,:]
  =
  \frac{1}{N_{\mathrm{tok}}}
  \sum_{n=1}^{N_{\mathrm{tok}}}
  \bm{R}^{\mathrm{L}}_{p}[b,n,:],
  \qquad
  \bm{q}^{\mathrm{R}}_{p}[b,:]
  =
  \frac{1}{N_{\mathrm{tok}}}
  \sum_{n=1}^{N_{\mathrm{tok}}}
  \bm{R}^{\mathrm{R}}_{p}[b,n,:],
\]
and then $\ell_2$-normalized:
\[
  \hat{\bm{q}}^{\mathrm{L}}_{p}[b,:]
  =
  \frac{\bm{q}^{\mathrm{L}}_{p}[b,:]}
       {\|\bm{q}^{\mathrm{L}}_{p}[b,:]\|_2+\varepsilon},
  \qquad
  \hat{\bm{q}}^{\mathrm{R}}_{p}[b,:]
  =
  \frac{\bm{q}^{\mathrm{R}}_{p}[b,:]}
       {\|\bm{q}^{\mathrm{R}}_{p}[b,:]\|_2+\varepsilon}.
\]
...
\[
  c_p(b)
  =
  \hat{\bm{q}}^{\mathrm{L}}_{p}[b,:]^\top
  \hat{\bm{q}}^{\mathrm{R}}_{p}[\pi_p(b),:].
\]
...
\begin{enumerate}
  ...
  \item Uniformly sample one consecutive window $\mathcal{W}$ of size $m$, token-normalize it, average it, detach the mean activation, and compute a rank-$k$ shared subspace via truncated SVD.
  ...
  \item Independently sample random layer pairs from the full depth, token-normalize both sides, subtract $\bm{A}^{\mathrm{com}}_{\mathrm{mean}}$, global-average-pool over tokens, batch-shuffle one side with a non-zero cyclic shift, and compute the cosine-squared private loss.
  \item Form
  \[
    \mathcal{L}
    =
