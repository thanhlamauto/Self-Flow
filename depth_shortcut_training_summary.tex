...
$p_{\mathrm{skip}}$, sample a short skip pair
\[
    b=a+d_{\mathrm{skip}},\qquad d_{\mathrm{skip}}=2
\]
by default. Build the source using detached hidden statistics for stability:
\[
...
\]
Log $\mathcal{L}_{\mathrm{skip-FM}}$, the effective skip probability, and the
sampled source/target layers. If full-backbone FID degrades, reduce
$p_{\mathrm{skip}}$ or $\lambda_{\mathrm{skip}}$ before increasing skip gap.
