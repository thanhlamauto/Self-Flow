"""Host dataloaders for MDM-style motion arrays and HumanML3D."""
...
def load_humanml_dataset(
    ...
    *,
    ...
    """Load native HumanML3D into MDM convention ``[N, 263, 1, 196]``."""
    ...
    if not os.path.isfile(split_file):
        raise FileNotFoundError(
            ...
            "Expected a preprocessed HumanML3D root with split files and new_joint_vecs."
        )
    ...
    if normalize:
        ...
        if not os.path.isfile(mean_path) or not os.path.isfile(std_path):
            raise FileNotFoundError(f"HumanML Mean.npy/Std.npy not found under {root}")
