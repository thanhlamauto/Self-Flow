...
def _build_repeat_segment_routes(segment_len: int, max_repeats: int) -> jax.Array:
    """Enumerate legal fixed-length routes with exactly one repeated block."""
    if segment_len < 2:
...
class SelfFlowDiT(nn.Module):
    ...
    def setup(self):
        ...
        if self.repeat_block_route:
            ...
            if self.repeat_window_len < 2:
                raise ValueError(
                    "repeat block window must contain at least 2 blocks, "
                    f"got {self.repeat_block_start}:{self.repeat_block_end}"
                )
    ...
    def _resolve_repeat_segment_route(
        ...
        if deterministic or not self.repeat_block_route or self.repeat_window_len == 0:
    ...
    def __call__(
        ...
        def capture_outputs(hidden, layer_idx_local, block_summaries_local, activations_local, zs_local):
            if return_block_summaries:
                block_summaries_local.append(jnp.mean(hidden, axis=1))
            if return_activations:
                activations_local.append(hidden)
            if layer_idx_local == return_features:
                zs_local = self.feature_head(hidden)
            elif layer_idx_local == return_raw_features:
                zs_local = hidden
            return block_summaries_local, activations_local, zs_local

        for block in self.blocks[:repeat_start]:
            x = block(x, c)
            layer_idx += 1
            block_summaries, activations, zs = capture_outputs(
                x,
                layer_idx,
                block_summaries,
                activations,
