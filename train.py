...
def sample_timestep_indices(
    ...
    """Sample discrete timestep indices in [0, shortcut_timesteps - 1]."""
    if sampling_mode == "logit_normal":
        ...
        tau = jax.nn.sigmoid(logits)
        q = jnp.rint(tau * max_idx.astype(jnp.float32)).astype(jnp.int32)
...
def train_step(
    ...
    *,
    ...
    timestep_pair_sampling_mode="source_uniform",
    ...
    if timestep_shortcut_loss_mode not in {"activation_huber", "direction_magnitude"}:
        raise ValueError(f"Unknown timestep shortcut loss mode: {timestep_shortcut_loss_mode!r}")
    pair_modes = {
        "trunc_normal",
    ...
    if timestep_pair_sampling_mode not in {"source_uniform", "target_uniform"}:
        raise ValueError(f"Unknown timestep pair sampling mode: {timestep_pair_sampling_mode!r}")
    ...
    timestep_paired_batch = bool(use_timestep_shortcut and timestep_paired_batch and not use_depth_time_resync)
    paired_local_batch = local_batch // 2
    if timestep_paired_batch and (local_batch % 2) != 0:
        raise ValueError("Timestep paired batch requires an even per-device local batch size.")
    ...
    if not (needs_ordered_timesteps and timestep_pair_sampling_mode == "target_uniform"):
    ...
    if needs_ordered_timesteps and timestep_pair_sampling_mode == "target_uniform":
        q_s = sample_timestep_indices(
            timestep_target_rng,
            local_batch,
            shortcut_timesteps,
            sampling_mode=timestep_sampling_mode,
    ...
    if timestep_paired_batch:
        x0_base = x0[:paired_local_batch]
        y_base = y[:paired_local_batch]
        q_base = q[:paired_local_batch]
        q_s_base = q_s[:paired_local_batch]
        q_r_base = q_r[:paired_local_batch]
        tau_base = tau[:paired_local_batch]
        tau_s_base = tau_s[:paired_local_batch]
        tau_r_base = tau_r[:paired_local_batch]
        x1_base = jax.random.normal(noise_rng, x0_base.shape)
        x_tau_high = (1.0 - tau_base[:, None, None]) * x1_base + tau_base[:, None, None] * x0_base
        x_tau_light = (1.0 - tau_s_base[:, None, None]) * x1_base + tau_s_base[:, None, None] * x0_base
        x_tau_r = (1.0 - tau_r_base[:, None, None]) * x1_base + tau_r_base[:, None, None] * x0_base
        x_tau = jnp.concatenate([x_tau_high, x_tau_light], axis=0)
        tau = jnp.concatenate([tau_base, tau_s_base], axis=0)
        q = jnp.concatenate([q_base, q_s_base], axis=0)
        y = jnp.concatenate([y_base, y_base], axis=0)
        target_base = x0_base - x1_base
        target = jnp.concatenate([target_base, target_base], axis=0)
    else:
        y_base = y
        q_base = q
        q_s_base = q_s
    ...
    def loss_fn(params):
        ...
        if timestep_paired_batch:
            pred_high = pred[:paired_local_batch]
            pred_light = pred[paired_local_batch:]
            hidden_stack_high_f32 = hidden_stack_f32[:, :paired_local_batch]
            hidden_stack_light_f32 = hidden_stack_f32[:, paired_local_batch:]
            dit_time_emb_high = dit_time_emb[:paired_local_batch]
            dit_time_emb_light = dit_time_emb[paired_local_batch:]
            h0_r_embed, dit_time_emb_r_embed = state.apply_fn(
                {"params": params["backbone"]},
                x_tau_r,
                timesteps=tau_r_base,
                vector=y_base,
                deterministic=True,
            ...
            pred_high = pred
            pred_light = pred
            hidden_stack_high_f32 = hidden_stack_f32
            hidden_stack_light_f32 = hidden_stack_f32
            dit_time_emb_high = dit_time_emb
            dit_time_emb_light = dit_time_emb
            h0_r_embed = None
            dit_time_emb_r_embed = None
        ...
        needs_r_forward = bool(
            ...
            or (timestep_bootstrap_requested and not timestep_paired_batch)
        )
        ...
        if timestep_paired_batch:
            pred_s = pred_light
            hidden_stack_s_f32 = hidden_stack_light_f32
            dit_time_emb_s = dit_time_emb_light
        elif needs_s_forward:
        ...
        def activation_huber_components(y_pred, target_hidden):
            target_hidden = jax.lax.stop_gradient(target_hidden.astype(jnp.float32))
            target_rms = jax.lax.stop_gradient(sample_activation_rms(target_hidden))
            residual_norm = (y_pred.astype(jnp.float32) - target_hidden) / target_rms
            loss_huber = jnp.mean(
                optax.huber_loss(
                    residual_norm,
                    delta=jnp.asarray(shortcut_activation_huber_delta, dtype=jnp.float32),
        ...
        def predictor_activation_transition(
            ...
            y_pred = state.predictor_apply_fn(
                ...
                jnp.asarray(timestep_shortcut_layer, dtype=jnp.int32),
                jnp.asarray(timestep_shortcut_layer, dtype=jnp.int32),
                src_embed,
                None,
                detach_timestep_embed=source_detach,
        ...
        def compute_output_distill_loss(_):
            ...
            output_a, output_b = output_as[0], output_bs[0]
        ...
        def compute_depth_time_resync_losses():
            ...
            return (
                ...
                e_dt_cons,
                e_out_time,
                legacy_direct_from_resync,
                loss_boot_from_resync,
                loss_boot_mag_from_resync,
                legacy_cos_dir_from_resync,
                cos_boot_from_resync,
                boot_target_rms_metric_local,
                boot_pred_rms_metric_local,
                boot_rel_residual_rms_metric_local,
        ...
        if use_depth_time_resync:
            (
                ...
                cos_dir_from_resync,
                cos_boot_from_resync,
                boot_target_rms_from_resync,
                boot_pred_rms_from_resync,
                boot_rel_residual_rms_from_resync,
            ) = compute_depth_time_resync_losses()
            loss_direct_joint = legacy_direct_from_resync
            loss_direct_ponly_1 = jnp.float32(0.0)
            loss_direct_ponly_2 = jnp.float32(0.0)
            loss_direct_3pair = legacy_direct_from_resync
            loss_dir = legacy_direct_from_resync
            loss_mag = jnp.float32(0.0)
            loss_dir_ponly_mean = jnp.float32(0.0)
            loss_mag_ponly_mean = jnp.float32(0.0)
            cos_dir = cos_dir_from_resync
            loss_boot = loss_boot_from_resync
            loss_boot_mag = loss_boot_mag_from_resync
            cos_boot = cos_boot_from_resync
            boot_target_rms_metric = boot_target_rms_from_resync
            boot_pred_rms_metric = boot_pred_rms_from_resync
            boot_rel_residual_rms_metric = boot_rel_residual_rms_from_resync
        else:
            (
                loss_depth_dt,
                loss_time_dt,
        ...
        legacy_direct_total = jnp.where(
            jnp.asarray(use_legacy_direct_loss, dtype=jnp.bool_),
            loss_direct_3pair,
            jnp.float32(0.0),
        )
        legacy_bootstrap_total = jnp.where(
            jnp.asarray(use_legacy_bootstrap_loss, dtype=jnp.bool_),
            lambda_boot * loss_boot + lambda_boot_mag * loss_boot_mag,
            jnp.float32(0.0),
        )
        legacy_shortcut_total = legacy_direct_total + legacy_bootstrap_total + loss_output_distill_weighted
        loss_total = (
            loss_gen
    ...
    metrics = {
        ...
        "train/timestep_pair_sampling_mode": jnp.asarray(
            1.0 if timestep_pair_sampling_mode == "target_uniform" else 0.0,
            dtype=jnp.float32,
        ),
        "train/timestep_sampling_mode": jnp.asarray(
            1.0 if timestep_sampling_mode == "logit_normal" else 0.0,
            dtype=jnp.float32,
        ),
...
class WandbRunSummaryFile:
    ...
    MIN_BEST_KEYS = {
        ...
        "val/FID_timestep_chain",
        "val/sFID_timestep_chain",
        "val/FID_timestep_fanout",
        "val/sFID_timestep_fanout",
...
def make_sample_latents_timestep_skip_pmap_fn(
    ...
    *,
    ...
    if eval_mode not in {"jump", "chain"}:
        raise ValueError(f"Unknown timestep skip eval_mode: {eval_mode!r}")
        ...
        def model_fn(z_x, t):
            ...
            def apply_timestep_jump(_):
                ...
                if eval_mode == "chain":
                    ...
                    pred_input = l2_normalize_tokens(src_hidden) if depth_shortcut_normalize_input else src_hidden
                    m_source = log_token_magnitudes(src_hidden)
                q_scale = jnp.float32(max(int(shortcut_timesteps) - 1, 1) / 0.96)
                q_src = jnp.rint(t * q_scale).astype(jnp.int32)
                q_tgt = jnp.rint(t_tgt * q_scale).astype(jnp.int32)
                q_src = jnp.clip(q_src, 0, int(shortcut_timesteps) - 1)
                ...
                if isinstance(pred_out, tuple):
                    y_pred, delta_m = pred_out
                else:
                    y_pred, delta_m = pred_out, None
...
def make_sample_latents_timestep_fanout_pmap_fn(
    config,
    *,
    num_steps=50,
    cfg_scale=1.0,
    ...
    def _sample_latents_local(ema_params_local, predictor_params_local, class_labels_local, rng_local):
        ...
        m_source = log_token_magnitudes(hidden_source)
        predictor_source = l2_normalize_tokens(hidden_source) if depth_shortcut_normalize_input else hidden_source
        ...
        def model_fn(z_x, t):
            ...
            if isinstance(hidden_next, tuple):
                y_pred, delta_m = hidden_next
            else:
                y_pred, delta_m = hidden_next, None
            if depth_shortcut_output_mode == "direction_activation":
                hidden_next = y_pred.astype(jnp.float32)
            else:
                m_pred = jnp.clip(
                    m_source + float(depth_shortcut_mag_scale) * delta_m,
                    float(depth_shortcut_mag_clip_min),
                ...
                hidden_next = jnp.exp(m_pred) * l2_normalize_tokens(y_pred)
            return model.apply(
                {"params": ema_params_local},
                z_x,
                timesteps=t,
...
def main():
    ...
    parser.add_argument(
        ...
        help="Timestep schedule for shortcut objectives. logit_normal samples sigmoid(N(mean, std)) then quantizes.",
    )
    ...
    parser.add_argument(
        "--timestep-pair-sampling-mode",
        type=str,
        default="source_uniform",
        choices=["source_uniform", "source-uniform", "target_uniform", "target-uniform"],
        help="For ordered timestep pairs, sample the source q first or sample the target q first to cover late targets.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Layer-index midpoint for centered pair sampling modes. Default is the middle of the model depth.",
    )
    parser.add_argument(
        "--pair-center-sigma",
        type=float,
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "For timestep shortcut training, use half as many unique images and create high/light "
            "noise pairs inside each local device batch."
        ),
    )
    parser.add_argument(
        "--timestep-shortcut-loss-mode",
        type=str,
        default="activation_huber",
        choices=["activation_huber", "direction_magnitude"],
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Condition the predictor on source/target layer ids for timestep shortcut. Default false because the layer is fixed.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--fid-timestep-skip-layer", type=int, default=9)
    ...
    args.timestep_pair_sampling_mode = args.timestep_pair_sampling_mode.replace("-", "_")
    ...
    if args.timestep_sampling_mode not in {"uniform", "logit_normal"}:
        raise ValueError("--timestep-sampling-mode must be uniform or logit_normal")
    if args.timestep_logit_std <= 0.0:
        raise ValueError("--timestep-logit-std must be greater than 0")
    if args.timestep_pair_sampling_mode not in {"source_uniform", "target_uniform"}:
        raise ValueError("--timestep-pair-sampling-mode must be source_uniform or target_uniform")
    ...
    if args.use_timestep_shortcut and args.timestep_paired_batch and (local_batch_size % 2) != 0:
        raise ValueError(
            "--use-timestep-shortcut with --timestep-paired-batch requires an even per-device local batch size; "
            f"got local batch {local_batch_size} from global batch {args.batch_size} and {num_devices} devices."
        )
    ...
    predictor_use_layer_cond = (
        bool(args.timestep_shortcut_use_layer_cond)
    ...
    depth_shortcut_predictor_overrides = {
        ...
        "depth_shortcut_dt_use_layer_cond": predictor_use_layer_cond,
    }
    ...
    if args.use_depth_time_resync:
        if args.use_depth_loss:
            active_losses.append("Ldepth")
        else:
            inactive_losses.append("Ldepth")
        if args.use_time_loss:
            active_losses.append("Ltime")
        else:
            inactive_losses.append("Ltime")
        if args.use_dt_loss:
            active_losses.append("Ldt")
        else:
            inactive_losses.append("Ldt")
        if args.use_depth_consistency:
            active_losses.append("Ldepth_cons")
        else:
            inactive_losses.append("Ldepth_cons")
        if args.use_time_consistency:
            active_losses.append("Ltime_cons")
        else:
            inactive_losses.append("Ltime_cons")
        if args.use_dt_consistency:
            active_losses.append("Ldt_cons")
        else:
            inactive_losses.append("Ldt_cons")
        if args.use_output_time_distill:
            active_losses.append("Lout_time")
        else:
            inactive_losses.append("Lout_time")
    else:
    ...
    if args.shortcut_lambda_skip_fm > 0.0 and args.shortcut_skip_in_loop_prob > 0.0:
        ...
        inactive_losses.append("Lskip_fm")
    if args.private_loss and args.lambda_private > 0.0:
    ...
    log_stage(
        ...
        f"timestep_pair_sampling={args.timestep_pair_sampling_mode} "
    ...
    state, ema_params, predictor_ema_params, l2_ema = create_train_state(
        ...
        dt_use_layer_cond=predictor_use_layer_cond,
    )
    ...
    effective_lambda_skip_fm = args.shortcut_lambda_skip_fm
    effective_skip_prob = args.shortcut_skip_in_loop_prob if args.shortcut_lambda_skip_fm > 0.0 else 0.0
    ...
    pmapped_train_step = jax.pmap(
        functools.partial(
            ...
            timestep_pair_sampling_mode=args.timestep_pair_sampling_mode,
        ),
        axis_name="batch",
    )
    ...
    fid_sample_latents_timestep_chain_pmapped = None
    if args.fid_timestep_chain_eval:
        fid_sample_latents_timestep_chain_pmapped = make_sample_latents_timestep_skip_pmap_fn(
            config,
            num_steps=args.fid_num_steps,
            cfg_scale=args.fid_cfg_scale,
            shortcut_predictor_variant=args.shortcut_predictor,
    ...
    fid_sample_latents_timestep_fanout_pmapped = None
    if args.fid_timestep_fanout_eval:
        fid_sample_latents_timestep_fanout_pmapped = make_sample_latents_timestep_fanout_pmap_fn(
            config,
            num_steps=args.fid_num_steps,
            cfg_scale=args.fid_cfg_scale,
            shortcut_predictor_variant=args.shortcut_predictor,
    ...
    def compute_eval_metrics(step, val_data_iter):
        ...
        if args.fid_timestep_chain_eval and fid_sample_latents_timestep_chain_pmapped is not None:
            ...
            chunk_idx_chain = 0
            chain_rng = jax.vmap(
                lambda key: jax.random.fold_in(key, jnp.uint32((step + 0xC1A1) & 0xFFFFFFFF))
            )(rng)
            while produced_chain < need:
                valid = min(global_b, need - produced_chain)
                class_rng, sample_rng = make_eval_chunk_rngs(chain_rng, chunk_idx_chain)
                classes = jax.vmap(
                    lambda key: jax.random.randint(key, (local_b,), 0, 1000),
                    in_axes=0,
                )(class_rng)
                latents_sharded = fid_sample_latents_timestep_chain_pmapped(
                    ema_params,
                    predictor_ema_params,
                    classes,
                    sample_rng,
                ...
                acc_chain_pooled, acc_chain_spatial = _update_accumulators(
                    acc_chain_pooled, acc_chain_spatial, pooled, spatial, valid_mask
                )
                produced_chain += valid
                chunk_idx_chain += 1

            mu_chain, cov_chain, _ = finalize_gaussian_sums(acc_chain_pooled)
            mu_chain_s, cov_chain_s, _ = finalize_gaussian_sums(acc_chain_spatial)
        ...
        if args.fid_timestep_fanout_eval and fid_sample_latents_timestep_fanout_pmapped is not None:
            ...
            chunk_idx_fanout = 0
            fanout_rng = jax.vmap(
                lambda key: jax.random.fold_in(key, jnp.uint32((step + 0xFA07) & 0xFFFFFFFF))
            )(rng)
            while produced_fanout < need:
                valid = min(global_b, need - produced_fanout)
                class_rng, sample_rng = make_eval_chunk_rngs(fanout_rng, chunk_idx_fanout)
                classes = jax.vmap(
                    lambda key: jax.random.randint(key, (local_b,), 0, 1000),
                    in_axes=0,
                )(class_rng)
                latents_sharded = fid_sample_latents_timestep_fanout_pmapped(
                    ema_params,
                    predictor_ema_params,
                    classes,
                    sample_rng,
                ...
                acc_fanout_pooled, acc_fanout_spatial = _update_accumulators(
                    acc_fanout_pooled, acc_fanout_spatial, pooled, spatial, valid_mask
                )
                produced_fanout += valid
                chunk_idx_fanout += 1

            mu_fanout, cov_fanout, _ = finalize_gaussian_sums(acc_fanout_pooled)
            mu_fanout_s, cov_fanout_s, _ = finalize_gaussian_sums(acc_fanout_spatial)
        ...
        if "val/FID_timestep_chain" in metrics:
            summary_parts.append(f"FIDchain={metrics['val/FID_timestep_chain']:.2f}")
            summary_parts.append(f"sFIDchain={metrics['val/sFID_timestep_chain']:.2f}")
        if "val/FID_timestep_fanout" in metrics:
            summary_parts.append(f"FIDfanout={metrics['val/FID_timestep_fanout']:.2f}")
            summary_parts.append(f"sFIDfanout={metrics['val/sFID_timestep_fanout']:.2f}")
