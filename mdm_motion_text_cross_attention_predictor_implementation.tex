...
The implemented HumanML3D predictor block is temporal depthwise-separable
Conv1D followed by self-attention, cross-attention, and MLP. Conv1D runs along
the frame axis and leaves the first condition token unchanged.
...
For HumanML3D, make the cross-attention path the default depth-shortcut
predictor. The historical \texttt{hybrid\_deep\_12pct} name is kept as a
compatibility alias, but it now resolves to the same text-cross architecture as
\texttt{hybrid\_text\_cross\_10pct}.
...
Switch & Default \\
\midrule
\texttt{--shortcut-predictor} & \texttt{hybrid\_text\_cross\_10pct} or \texttt{hybrid\_deep\_12pct} \\
\texttt{--shortcut-predictor-use-timestep} & on \\
\texttt{context source} & masked MDM condition token $z_{tk}$ \\
\texttt{width} & $96$ \\
\texttt{blocks} & $8$ \\
\texttt{heads} & $4$ \\
\texttt{parameters} & $1.777$M, about $9.94\%$ of a $17.88$M MDM-B backbone \\
\bottomrule
\end{tabular}
\end{center}
