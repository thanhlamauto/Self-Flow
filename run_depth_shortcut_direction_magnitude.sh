...
# Usage:
#   ./run_depth_shortcut_direction_magnitude.sh
#   ./run_depth_shortcut_direction_magnitude.sh tiny small base large
#
# If no predictor variant is provided, this script runs P-Base, the balanced
# setting. Pass one or more variants to run a sweep on the same VM.
...
for predictor in "${PREDICTORS[@]}"; do
  python train.py \
    "${COMMON_ARGS[@]}" \
    "${DIRECTION_MAG_ARGS[@]}" \
    --ckpt-dir "./checkpoints-depth-shortcut-${predictor}-dir-mag" \
