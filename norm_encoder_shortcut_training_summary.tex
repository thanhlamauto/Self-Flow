...
\]
By default, bootstrap losses can backpropagate through the source hidden state
into the DiT backbone. Setting
\[
    \texttt{--shortcut-bootstrap-detach-source}
\]
uses \(\operatorname{sg}(U_a)\) and \(\operatorname{sg}(m_a)\) for the
bootstrap student path, so \(\mathcal{L}_{\mathrm{boot}}\) and
\(\mathcal{L}_{\mathrm{boot-mag}}\) update the predictor without updating the
backbone through the bootstrap source. Direct loss and skip-FM are unchanged.
